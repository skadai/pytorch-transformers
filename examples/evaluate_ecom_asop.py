...
def parse_args():
    ...
    parser.add_argument('--score_type', '-st', help="score type: aspect/opinion/polar")
...
def make_qid_to_has_ans(dataset, score_type=None, subtype=None):
    qid_to_has_ans = {}
    for example in dataset:
        if example.question_text == subtype:
            qid_to_has_ans[example.qas_id] =not example.is_op_impossible if score_type == 'opinion' else not example.is_impossible
...
def get_raw_scores(dataset, preds, score_type=None, subtype=None):
    exact_scores = {}
    f1_scores = {}
    raw_answers = {}
    for qa in dataset:
        ...
        elif score_type == 'polar':
            gold_answers = [qa.polar_answer_text if normalize_answer(qa.polar_answer_text) else '']
...
def main():
    ...
    exact_raw, f1_raw, raw_answers = get_raw_scores(dataset, preds, OPTS.score_type, subtype=subtype_cn)  # 返回的都是字典
